...
public class ClockSolver {
   ...
   *   @param args String array from the main method
   */
   public void handleInitialArguments( String args[] ) {
     ...
     // you may want to consider using args[2] for an "angle window"
      ...
      System.out.println( "\n   Hello world, from the ClockSolver program!!" +
                          "\n   This program is designed to take in an angle value and" +
                          "\n   a time slice (optional) and calculates the time at which" +
                          "\n   the clock (see Clock.java) forms the specified angle with" +
                          "\n   its minute and hour hands. The program then outputs the" +
                          "\n   time that the clock reads when this occurs.   \n" );
      if( 0 == args.length ) {
   ...
   public static void main( String args[] ) {
      ...
      while( cse.clock.getTotalSeconds() <=  cse.MAX_SECONDS ) {
        ...
        if (Math.abs(angleComp - desiredAngle) <= cse.EPSILON_VALUE ||
          ...
          System.out.println("TIME: " + cse.clock.toString() + "  -----   " + angleComp + "  is within range of desiredAngle: " + desiredAngle + "-------------------------\n");
        }
        cse.clock.tick();
      }
      System.exit( 0 );
